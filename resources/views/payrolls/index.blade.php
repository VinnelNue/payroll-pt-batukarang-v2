...
<div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
    ...
    <!-- 2. SUMMARY CARDS ELEGAN (HANYA MUNCUL UNTUK MANAGER KEUANGAN/SUPER ADMIN) -->
    @if($isExecutive)
        @php
        ...
        <table class="table table-hover align-middle mb-0 text-nowrap">
            <thead class="bg-light border-bottom">
                <tr class="text-secondary small fw-bold text-uppercase">
                    ...
                    <!-- BPJS DITAMPILKAN UNTUK SEMUA (HRD JUGA BISA LIHAT KETERANGANNYA) -->
                    <th class="py-3 px-3 text-end">BPJS TK (2%)</th>
                    <th class="py-3 px-3 text-end">BPJS KS (1%)</th>
                    
                    <!-- HEADER FINANSIAL LAINNYA HANYA MUNCUL UNTUK MANAGER KEUANGAN -->
                    @if($isExecutive)
            ...
            <tbody class="divide-y">
                ...
                @php
                    // Logika Pendeteksian Jabatan untuk sensor nominal BPJS jika HRD yang lihat
                    $jabatan = strtolower($pay->employee->activeContract->job_title ?? '');
                    $isHighLevel = str_contains($jabatan, 'manager') || 
                                   str_contains($jabatan, 'kepala') || 
                                   str_contains($jabatan, 'hrd') || 
                                   str_contains($jabatan, 'direktur');

                    $canViewBpjsNominal = $isExecutive || !$isHighLevel;
                @endphp
                <tr>
                    <td class="text-center fw-bold text-muted px-3">{{ $index + 1 }}</td>
                    
                    <!-- NAMA DAN POSISI PEKERJAAN -->
                    <td class="px-3">
                        ...
                        @else
                            <span class="badge bg-danger-subtle text-danger border border-danger-subtle small fw-normal mt-1">
                                <i class="fa-solid fa-lock" style="font-size: 10px;"></i> Posisi Dirahasiakan
                            </span>
                        @endif
                    </td>
                    ...
                    <!-- BPJS TK (2%) (DITAMPILKAN KE SEMUA, TAPI DISENSOR JIKA ATASAN) -->
                    <td class="text-end px-3">
                        ...
                        @if($pay->is_bpjs_override)
                            <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2" style="font-size: 0.65rem;" title="Nilai diinput manual (Override)">
                                <i class="fa-solid fa-pen-to-square me-1"></i> Override
                            </span>
                        @else
                            <span class="badge bg-success-subtle text-success border border-success-subtle px-2" style="font-size: 0.65rem;" title="Dihitung otomatis oleh sistem">
                                <i class="fa-solid fa-robot me-1"></i> Auto
                            </span>
                        @endif
                    </td>

                    <!-- BPJS KS (1%) (DITAMPILKAN KE SEMUA, TAPI DISENSOR JIKA ATASAN) -->
                    <td class="text-end px-3">
                        @if($canViewBpjsNominal)
                            <div class="text-muted small fw-semibold mb-1">
                    ...
                    <!-- KOLOM FINANSIAL LAINNYA HANYA MUNCUL UNTUK MANAGER KEUANGAN -->
                    @if($isExecutive)
                ...
                @empty
                <tr>
                    <td colspan="{{ $isExecutive ? 10 : 5 }}" class="text-center py-5 text-muted">
